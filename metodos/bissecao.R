bissection <- function(env_funcao, env_pontos_1, env_pontos_2, env_decimais, env_iteracoes, g_indices, g_lh) {
  ...
  tryCatch({
    ...
    if (is.null(error_vector)) {
      ...
      p <- ggplot(data = data.frame(x = 0, y = 0), mapping = aes(x = x, y = y)) +
        geom_vline(xintercept = 0) + geom_hline(yintercept = 0) + xlim(a_k[1] - h_x, b_k[1] + h_x) + ylim(y_min, y_max)
      p <- p + stat_function(fun = func, col = "red")
      # Salvar o plot na lista
      plot_vector <<- list(p)
      
      ### Plot dos pontos a e b sobre o eixo x
      plot_vector[[2]] <<- p + geom_point(x = a_k[1], y = 0, col = "blue", pch = 1) +
        geom_point(x = b_k[1], y = 0, col = "blue", pch = 1) +
        annotate("text", label = "a0", x = a_k[1], y = h_ind) + annotate("text", label = "b0", x = b_k[1], y = h_ind)
      ...
      for (i in 1:cont) {
        ...
        if (g_lh) {
          ...
          plot_vector[[length(plot_vector) + 1]] <<- plot_vector[[length(plot_vector)]] +
            geom_segment(x = inter_x[1], xend = inter_x[2], y = y_min, yend = y_min, size = i + (i - 1) * 0.3, col = "green4") +
            geom_point(x = inter_x[1], y = y_min, shape = 21, bg = "green4") +
            geom_point(x = inter_x[2], y = y_min, shape = 21, bg = "green4")
        }
        # Indices dos pontos
        if (g_indices) {
          plot_vector[[length(plot_vector) + 1]] <<- plot_vector[[length(plot_vector)]] +
            geom_point(x = m_k[i], y = 0, col = "blue", shape = 1) + annotate("text", label = toString(i - 1), x = m_k[i], y = h_ind)
        }
        else{
          # Plot dos pontos m_k sobre o eixo x
          plot_vector[[length(plot_vector) + 1]] <<- plot_vector[[length(plot_vector)]] +
            geom_point(x = m_k[i], y = 0, col = "blue", shape = 1)
        }
      }
      value_output <<- list()
