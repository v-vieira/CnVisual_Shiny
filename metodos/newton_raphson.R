newtonraphson <- function(env_funcao, env_x0, env_decimais, env_iteracoes, g_indices, g_lv, g_ltg) {
  ...
  tryCatch({
    ...
    tryCatch({
      ...
      dev_x0 <- dev_func(x0)   #valor da F"(x0)
    },
    warning = function(w) {
      if (is.null(error_vector)) {
    ...
    if (abs(func(x0)) < stp) {
      ...
      whilevar <- - 1
        ...
        while (whilevar == - 1) {
          ...
          if (abs(x_k[cont] - x_k[cont - 1]) < stp) {
            whilevar <- 1
          }
      ...
      for (i in 1:cont) {
        ...
        if (g_lv) {
          plot_vector[[length(plot_vector) + 1]] <<- plot_vector[[length(plot_vector)]] +
            geom_segment(x = x_k[i], xend = x_k[i], y = 0, yend = fx_k[i], col = "darkgray", linetype = "dashed")
        }
        ...
        if (g_ltg) {
          plot_vector[[length(plot_vector) + 1]] <<- plot_vector[[length(plot_vector)]] +
            geom_segment(x = x_k[i], xend = x_k[i + 1], y = fx_k[i], yend = 0, col = "black")
        }
      }
      value_output <<- list()
